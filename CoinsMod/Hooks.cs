...
namespace SimpleLunarCoins
{
	public class Hooks
	{
        public static void Init()
        {
            ...
            {
                ProperSave.SaveFile.OnGatherSaveData += SaveCoins;
            }
        }
        ...
        */
        ...
        private static void StartingCoins(On.RoR2.Run.orig_Start orig, Run self)
        {
            ...
            if (ProperSaveCompatibility.enabled)
            {
                if (ProperSave.Loading.IsLoading) { isLoading = true; }
            }
            ...
            {
                string jsonString = ProperSave.Loading.CurrentSave.GetModdedData<string>("SimpleLunarCoinsObj");
                ...
                foreach (var user in NetworkUser.instancesList)
                {
                    ...
                    {
                        user.InvokeMethod("RpcDeductLunarCoins", user.lunarCoins);
                        user.InvokeMethod("RpcAwardLunarCoins", (uint)playerCoins[user.GetNetworkPlayerName().steamId.steamValue]);
                    }
                }
            }
            orig(self);
